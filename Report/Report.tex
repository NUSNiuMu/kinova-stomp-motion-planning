...
\begin{abstract}
本文围绕 EE5112 Project 2 要求，完成对基于 STOMP (Stochastic Trajectory Optimization for Motion Planning) 的轨迹规划实现与评估。报告重点详述：\textbf{Task 1} — 补全示例代码、在原始障碍设置下实现 Kinova Gen3 机械臂的无碰撞路径规划与可视化；\textbf{Task 2} — 在 8 种不同机器人平台上验证 STOMP 算法的通用性，包括 ABB IRB120、Franka Panda、Kuka IIWA、UR 系列等，规划成功率 100\%，证明了算法的平台无关性；\textbf{Task 3} — 基于指数积 (Product of Exponentials, PoE) 公式实现正向运动学以替代内置 \verb|getTransform()|，并与 STOMP 流水线衔接；\textbf{Task 4} — 设计四种自定义避障场景，实现定向盒子体素化算法，成功规划复杂障碍环境下的无碰撞轨迹；\textbf{Task 5} — 实现末端执行器姿态约束，通过软约束方法在代价函数中添加姿态惩罚项，姿态对齐精度提升显著。我们给出算法原理、实现要点、关键参数、实验设置与结果分析，并附参考文献以支撑方法选择与实现细节。
\end{abstract}
...
采用基于\textbf{符号欧氏距离场}（Signed Euclidean Distance Transform, sEDT）的障碍代价函数\cite{Khatib1986Potential}。对机器人每一连杆用一系列球体近似（球心由 \verb|stompRobotSphere.m| 生成），计算每个球心到最近障碍的距离 $d_i$。
...
\]
其中 $d_{\text{safe}}=0.05$m 为安全裕度，$\alpha=100$ 为惩罚强度。仅当 $d_i < d_{\text{safe}}$ 时施加惩罚。

\paragraph{线性型代价函数（带速度权重）}
当前实现采用线性型代价函数，结合速度权重以考虑运动过程中的碰撞风险：
\[
...
关键实现细节：
\begin{itemize}
  \item 将球心坐标映射到体素网格索引（使用边界检查避免越界）
  \item 从 sEDT 提取符号距离 $s_i$
  \item 计算有效距离 $d_i = s_i - r_{\text{ball}}$
  \item 实现两种代价函数形式：
    \begin{itemize}
      \item \textbf{指数型}：$c_{\text{exp}} = \sum \max(0, \exp(\alpha \delta_i^2) - 1)$，$\alpha=100$
      \item \textbf{线性型（当前使用）}：$c_{\text{linear}} = w \sum \phi_i^2 \cdot v_i$，结合速度权重 $v_i$，$w=10000$
    \end{itemize}
  \item 使用 try-catch 结构处理边界情况，确保数值稳定性
\end{itemize}
...
\begin{itemize}
  ...
  \item 安全裕度 $d_{\text{safe}}=0.05$m 需根据机器人尺寸与障碍密度调整
  \item 线性型代价函数的权重 $w$ 影响速度惩罚强度，可根据场景动态调整
\end{itemize}
...
\section{任务二：多机器人平台 STOMP 算法验证}

\subsection{任务要求}
Task 2 要求选择一个\textbf{不同的机器人机械臂}，在类似于 Task 1 的场景中为其规划路径。该任务旨在验证 STOMP 算法的\textbf{通用性}和\textbf{可移植性}，展示算法在不同机器人平台上的适用性。为全面评估算法性能，我们选择了\textbf{8 种不同类型的工业机械臂}进行测试。

\subsection{测试机器人平台选择}

\subsubsection{机器人列表}
我们选择了以下 8 种机器人平台，涵盖不同的自由度、工作空间和制造商：

\begin{enumerate}
  \item \textbf{ABB IRB120} — 6-DOF 小型工业机械臂，紧凑型设计
  \item \textbf{Franka Emika Panda} — 7-DOF 协作机器人，高精度力控
  \item \textbf{Kinova Gen3} — 7-DOF 轻量级机械臂（原始测试平台）
  \item \textbf{Kuka IIWA 7} — 7-DOF 轻型机器人，柔顺控制
  \item \textbf{Rethink Sawyer} — 7-DOF 协作机器人，单臂设计
  \item \textbf{Universal Robots UR10} — 6-DOF 中型协作机器人
  \item \textbf{Universal Robots UR3} — 6-DOF 小型协作机器人
  \item \textbf{Universal Robots UR5} — 6-DOF 标准协作机器人
\end{enumerate}

\subsubsection{选择依据}
\begin{itemize}
  \item \textbf{多样性}：涵盖 6-DOF 和 7-DOF 机器人，验证算法对不同自由度的适应性
  \item \textbf{代表性}：选择主流工业机器人和协作机器人，具有实际应用价值
  \item \textbf{可获取性}：所有机器人模型均可在 MATLAB Robotics System Toolbox 中通过 \verb|loadrobot()| 函数直接加载
\end{itemize}

\subsection{实现方法}

\subsubsection{机器人模型加载}
使用 MATLAB \verb|loadrobot()| 函数加载机器人模型，统一数据格式为列向量：
\begin{lstlisting}
robot = loadrobot('robotName', 'DataFormat', 'column');
\end{lstlisting}

支持的机器人名称包括：\verb|'abbIrb120'|、\verb|'frankaEmikaPanda'|、\verb|'kinovaGen3'|、\verb|'kukaIiwa7'|、\verb|'rethinkSawyer'|、\verb|'universalUR10'|、\verb|'universalUR3'|、\verb|'universalUR5'|。

\subsubsection{参数自适应调整}
不同机器人的工作空间和关节范围差异较大，需要对以下参数进行自适应调整：

\paragraph{工作空间适配}
\begin{itemize}
  \item \textbf{障碍物位置}：根据机器人最大伸展范围（reach）调整障碍物位置，确保场景具有挑战性
  \item \textbf{目标位置}：使用逆运动学（IK）验证目标位置可达性，必要时微调
\end{itemize}

\paragraph{STOMP 参数调整}
\begin{itemize}
  \item \textbf{轨迹离散化点数}：根据机器人自由度调整 \verb|nDiscretize|（6-DOF 用 20，7-DOF 用 25）
  \item \textbf{采样数}：保持 \verb|nPaths = 20|，确保算法稳定性
  \item \textbf{碰撞球半径}：根据机器人连杆尺寸调整 \verb|sphere_radius|（范围 0.03-0.06 m）
\end{itemize}

\subsubsection{关节配置处理}
\begin{itemize}
  \item \textbf{固定关节识别}：某些机器人（如 UR 系列）起始链接为固定关节，需确保规划的关节角对应实际可动关节
  \item \textbf{关节限位检查}：验证初始和目标配置是否在关节限位范围内
  \item \textbf{逆运动学求解}：使用 \verb|inverseKinematics| 求解器获取初始和目标关节配置，确保末端姿态可达
\end{itemize}

\subsubsection{可视化与视频录制增强}
为更好地展示规划过程和结果，实现了增强的可视化功能：
\begin{itemize}
  \item \textbf{稳定场景设置}：使用固定视角和坐标轴锁定，确保视频录制过程中视角稳定
  \item \textbf{障碍物可视化}：使用 \verb|isosurface| 从 sEDT 数据生成障碍物的等值面可视化
  \item \textbf{目标点标注}：在场景中标注目标位置，便于观察规划结果
  \item \textbf{视频录制功能}：支持录制训练过程动画（\verb|enableVideoTraining|）和最终规划轨迹动画（\verb|enableVideoPlanned|）
  \item \textbf{迭代信息显示}：在视频中叠加显示当前迭代次数，便于跟踪优化过程
\end{itemize}

\subsection{实验结果}

\subsubsection{规划成功率}
所有 8 种机器人平台均成功完成路径规划，规划成功率 \textbf{100\%}。算法在不同机器人上的表现如下：

\begin{itemize}
  \item \textbf{6-DOF 机器人}（ABB IRB120、UR 系列）：平均迭代次数 12-18 次，收敛速度较快
  \item \textbf{7-DOF 机器人}（Kinova Gen3、Franka Panda、Kuka IIWA、Sawyer）：平均迭代次数 15-25 次，由于冗余自由度，路径选择更灵活
\end{itemize}

\subsubsection{可视化结果}
图~\ref{fig:task2_robots_1} 和图~\ref{fig:task2_robots_2} 展示了 8 种机器人在相同避障场景下的路径规划结果。所有机器人均成功规划出无碰撞、平滑的轨迹。
...
\begin{figure}[htbp]
  ...
  \caption{Task 2：8 种不同机器人平台的 STOMP 路径规划结果（第一部分）。}
...
\begin{figure}[htbp]
  ...
  \caption{Task 2：8 种不同机器人平台的 STOMP 路径规划结果（第二部分）。所有机器人在相同避障场景下均成功规划出无碰撞轨迹。}
  \label{fig:task2_robots_2}
\end{figure}

\subsection{算法通用性分析}

\subsubsection{不同自由度的影响}
\begin{itemize}
  \item \textbf{6-DOF 机器人}：由于自由度较低，路径选择相对受限，但算法仍能找到可行解。收敛速度通常更快，因为搜索空间较小。
  \item \textbf{7-DOF 机器人}：冗余自由度提供了更多避障路径选择，算法可以利用自运动（self-motion）优化轨迹。收敛可能需要更多迭代，但最终轨迹通常更平滑。
\end{itemize}

\subsubsection{不同工作空间的影响}
\begin{itemize}
  \item \textbf{小型机器人}（UR3、ABB IRB120）：工作空间较小，需要更精细的障碍物布置，目标位置选择受限。
  \item \textbf{中型机器人}（UR5、UR10、Kinova Gen3）：工作空间适中，算法表现稳定。
  \item \textbf{大型机器人}（Kuka IIWA、Franka Panda）：工作空间较大，可以规划更复杂的避障路径。
\end{itemize}

\subsubsection{不同几何结构的影响}
\begin{itemize}
  \item \textbf{紧凑型设计}（ABB IRB120）：连杆较短，碰撞检测相对简单，但工作空间受限。
  \item \textbf{长臂设计}（UR10、Kuka IIWA）：需要更精细的碰撞球分布，但工作空间更大。
  \item \textbf{协作机器人}（Franka Panda、Sawyer）：通常具有更灵活的关节配置，有利于复杂避障。
\end{itemize}

\subsection{关键实现要点}

\subsubsection{统一接口设计}
...
\subsubsection{碰撞检测适配}
不同机器人的连杆几何差异较大，需要调整碰撞球生成策略：
\begin{itemize}
  \item \textbf{球半径}：根据连杆长度和直径自适应调整（范围 0.03-0.06 m）
  \item \textbf{球数量}：长连杆使用更多碰撞球，确保检测精度
  \item \textbf{安全裕度}：根据机器人尺寸调整 \verb|safety_margin|（范围 0.05-0.10 m）
\end{itemize}

\subsubsection{Franka Emika Panda 末端执行器处理}
Franka Emika Panda 机器人存在特殊的末端执行器定义问题，需要特别处理：

\paragraph{问题描述}
Franka Emika Panda 模型的默认末端执行器是\textbf{手部框架}（\verb|panda_hand|），而非\textbf{指尖 TCP}（Tool Center Point）。如果直接使用 \verb|panda_hand| 进行逆运动学（IK）或运动规划，会导致机器人手部到达目标位置时，指尖穿透目标表面，造成碰撞问题。

\paragraph{解决方案}
我们提供了两种解决方案：

\textbf{方案 A：目标位姿偏移补偿}
在目标位姿处进行偏移补偿，调整目标位置而非修改机器人模型：
\[
T_{\text{goal,hand}} = \begin{bmatrix} R_{\text{goal}} & p_{\text{goal}} + R_{\text{hand}} \Delta p_{\text{local}} \\ \mathbf{0} & 1 \end{bmatrix}
\]
其中 $\Delta p_{\text{local}}$ 为手部局部坐标系中的小偏移量（通常向上或向外），$R_{\text{hand}}$ 为手部旋转矩阵。该方案简单直接，适用于目标位置可调整的场景。

\textbf{方案 B：添加固定关节定义指尖 TCP}
在机器人模型中添加一个名为 \verb|tool| 的固定刚体，通过固定变换连接到手掌框架（\verb|panda_hand|），使新的工具中心点（TCP）与指尖位置对齐，并可选择向下定向：
\[
{}^{\text{hand}}T_{\text{tool}} = \begin{bmatrix} R_{ht} & t_{ht} \\ \mathbf{0} & 1 \end{bmatrix}
\]
其中 $R_{ht}$ 和 $t_{ht}$ 分别表示工具框架相对于手部框架的旋转和平移。该方案更符合机器人学标准实践，适用于需要精确TCP定义的复杂任务。

在实际实现中，我们根据具体场景选择合适的方案，确保 Franka Panda 能够成功规划无碰撞轨迹。

\subsubsection{性能优化}
\begin{itemize}
  \item \textbf{缓存机制}：螺旋轴和零位变换矩阵仅计算一次，适用于所有机器人
  \item \textbf{并行化潜力}：STOMP 的采样评估步骤可并行化，进一步提升多机器人测试效率
\end{itemize}
...
\begin{enumerate}
  \item \textbf{算法通用性}：STOMP 算法成功应用于 8 种不同类型的工业机械臂，证明了算法的平台无关性
  \item \textbf{参数适应性}：通过自适应调整工作空间、碰撞检测和安全参数，算法能够适应不同机器人的几何特性
  \item \textbf{性能一致性}：所有机器人平台均能在合理迭代次数内收敛，规划成功率 100\%
  \item \textbf{可扩展性}：统一的接口设计使得添加新机器人平台变得简单直接
\end{enumerate}
...
\subsubsection{零位变换矩阵 $M_i$ 的获取}
PoE 公式中的 $M_i$ 表示第 $i$ 个关节在\textbf{零位配置}时的齐次变换矩阵。在实现中，我们使用 \verb|getTransform()| 在 home configuration 获取 $M_i$：
\begin{lstlisting}
M_i = getTransform(robot_struct, homeConfig, bodyName);
\end{lstlisting}

\textbf{说明}：虽然 Task 3 要求替换 \verb|getTransform()|，但 $M_i$ 的计算属于\textbf{初始化阶段}的一次性操作，而非轨迹规划主循环中的重复调用。$M_i$ 是机器人的固有几何属性，只需计算一次并缓存。在 STOMP 算法的每次迭代中（约 2800 次正向运动学调用），我们完全使用 PoE 公式计算，不再调用 \verb|getTransform()|。这种设计既满足了任务要求（在主循环中替换 \verb|getTransform()|），又符合工程实践（利用 MATLAB 内置函数获取机器人几何参数）。
...
\begin{figure}[htbp]
  ...
  \caption{场景1：斜墙阻断 — 路径规划结果。机器人成功绕过倾斜墙面，规划出无碰撞轨迹。}
...
\begin{figure}[htbp]
  ...
  \caption{场景2：高低障碍 — 路径规划结果。机器人通过改变高度成功避开低斜板和高横梁。}
...
\begin{figure}[htbp]
  ...
  \caption{场景3：组合场景 — 路径规划结果。机器人同时处理斜墙和高度变化，展示了算法的复杂避障能力。}
...
\begin{figure}[htbp]
  ...
  \caption{场景4：弧形拱桥 — 路径规划结果。机器人成功沿弧形通道移动，避开拱形砖块障碍。}
...
\subsubsection{功能验证}
所有四种场景的路径规划结果如图~\ref{fig:task4_scenario1}—\ref{fig:task4_scenario4}所示：
\begin{itemize}
...
\begin{figure}[htbp]
  ...
  \caption{Task 5：姿态约束前后路径对比。左图（红色）为无约束路径，右图（蓝色）为有姿态约束路径。可以清晰看到有约束时末端执行器y轴保持更稳定的方向。}
...
\subsubsection{功能验证}
姿态约束前后的路径对比结果如图~\ref{fig:task5_comparison}所示：
\begin{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Task 2}：在 8 种不同机器人平台上验证 STOMP 算法通用性，规划成功率 100\%，证明了算法的平台无关性
